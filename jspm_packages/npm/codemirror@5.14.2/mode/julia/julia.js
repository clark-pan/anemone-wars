...
(function(mod) {
  ...
  CodeMirror.defineMode("julia", function(_conf, parserConf) {
    ...
    function tokenBase(stream, state) {
      ...
      var isDefinition = state.isDefinition || state.lastToken == 'function' || state.lastToken == 'macro' || state.lastToken == 'type' || state.lastToken == 'immutable';
    ...
    var external = {
      ...
      token: function(stream, state) {
        ...
        if (current === '.') {
          style = stream.match(identifiers, false) || stream.match(macro, false) || stream.match(/\(/, false) ? 'operator' : ERRORCLASS;
        }
        return style;
      },
